---
name: blog-link-extraction-subagent-custom-agent
...
3. **Link Extraction**: Use the `link_extractor` custom agent for each found blog post and run all extraction agents in parallel. Each agent extracts and filters links for exactly one post; then aggregate their results without user interaction to provide a complete result set.
...
5. For every extraction, invoke the existing `link_extractor` custom agent with exactly one assigned blog post URL and the requested topic.
...
1. After the blog post list is finalized and de-duplicated, invoke exactly one `link_extractor` custom agent per blog post. Launch all of these agents in the same turn so the posts are processed in parallel.
2. Give each agent exactly one assigned blog post URL and the requested topic. The agent must:
